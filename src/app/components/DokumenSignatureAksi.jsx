...
export default function DokumenSignatureAksi({ dokumen, refId, onCetakFisik, hideCetakFisik }) {
  ...
  async function tandaiSelesaiMock() {
    setLoading(true); setError(null);
    try {
      const res = await fetch(`/api/dokumen-signature/${sig.id}/selesaikan`, { method: 'POST' });
      const d = await res.json();
  ...
  const warna = sig ? (FASE_WARNA[sig.fase] || { bg: '#F5F8FE', fg: '#1A4FA0' }) : null;
  ...
  return (
    <div className="no-print" style={{ textAlign: 'center', marginBottom: 16 }}>
      <div style={{ display: 'flex', gap: 8, justifyContent: 'center', flexWrap: 'wrap' }}>
        ...
        <button onClick={kirimDigital} disabled={loading || sig?.metode === 'digital'}
      ...
      {sig?.metode === 'digital' && (
        <div style={{ display: 'inline-block', marginTop: 10, padding: '8px 16px', borderRadius: 10, fontSize: 12, background: warna.bg, color: warna.fg }}>
          ...
          {sig.fase === 'ttd_menunggu' && (
            <div style={{ marginTop: 6, display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', justifyContent: 'center' }}>
              ...
              <button onClick={tandaiSelesaiMock} disabled={loading}
