...
const KATEGORI_VALID = ['merchandise', 'sewa_aset', 'lain_lain'];

// GET /api/admin/pendapatan-lain?from=&to=&kategori= — khusus super admin
  ...
  try {
    ...
    const [rows] = await pool.query(
      `SELECT p.*, u.name AS input_oleh_nama
       FROM pendapatan_lain p LEFT JOIN users u ON u.id = p.input_oleh
       ${where} ORDER BY p.tanggal DESC, p.created_at DESC`,
      params
    );
    return Response.json({ pendapatan: rows });
  } catch (error) {
    console.error(error);
...
export async function POST(request) {
  ...
  try {
    ...
    const [result] = await pool.query(
      'INSERT INTO pendapatan_lain (tanggal, kategori, keterangan, nominal, bukti_path, bukti_nama, input_oleh) VALUES (?, ?, ?, ?, ?, ?, ?)',
      [tanggal, kategoriFinal, keterangan?.trim() || null, Number(nominal), bukti_path || null, bukti_nama || null, auth.user.id]
    );
    return Response.json({ message: 'Pendapatan lain dicatat!', id: result.insertId });
  } catch (error) {
    console.error(error);
    return Response.json({ error: 'Terjadi kesalahan server' }, { status: 500 });
  }
...
export async function PUT(request) {
  ...
  try {
    ...
    const kategoriFinal = KATEGORI_VALID.includes(kategori) ? kategori : 'lain_lain';
    const [result] = await pool.query(
      'UPDATE pendapatan_lain SET tanggal = ?, kategori = ?, keterangan = ?, nominal = ?, bukti_path = ?, bukti_nama = ? WHERE id = ?',
    ...
    return Response.json({ message: 'Pendapatan lain diperbarui!' });
  } catch (error) {
    console.error(error);
    return Response.json({ error: 'Terjadi kesalahan server' }, { status: 500 });
  }
...
export async function DELETE(request) {
  ...
  try {
    const [result] = await pool.query('DELETE FROM pendapatan_lain WHERE id = ?', [id]);
    if (result.affectedRows === 0) {
      return Response.json({ error: 'Data tidak ditemukan' }, { status: 404 });
    }
    return Response.json({ message: 'Pendapatan lain dihapus!' });
  } catch (error) {
    console.error(error);
    return Response.json({ error: 'Terjadi kesalahan server' }, { status: 500 });
  }
