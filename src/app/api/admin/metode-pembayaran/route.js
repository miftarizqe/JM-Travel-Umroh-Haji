...
export async function POST(request) {
  ...
  if (auth.error) return auth.error;
  try {
    const { nama, nomor, atas_nama, catatan, khusus_sahabat } = await request.json();
    if (!nama?.trim()) {
    ...
    const [result] = await pool.query(
      'INSERT INTO metode_pembayaran (urutan, nama, nomor, atas_nama, catatan, khusus_sahabat) VALUES (?, ?, ?, ?, ?, ?)',
      [maxUrutan + 1, nama.trim(), nomor?.trim() || null, atas_nama?.trim() || null, catatan?.trim() || null, khusus_sahabat ? 1 : 0]
    );
...
export async function PUT(request) {
  ...
  if (auth.error) return auth.error;
  try {
    const { id, nama, nomor, atas_nama, catatan, aktif, khusus_sahabat } = await request.json();
    if (!id || !nama?.trim()) {
      return Response.json({ error: 'Data tidak lengkap' }, { status: 400 });
    }
    const [result] = await pool.query(
      'UPDATE metode_pembayaran SET nama = ?, nomor = ?, atas_nama = ?, catatan = ?, aktif = ?, khusus_sahabat = ? WHERE id = ?',
      [nama.trim(), nomor?.trim() || null, atas_nama?.trim() || null, catatan?.trim() || null, aktif ? 1 : 0, khusus_sahabat ? 1 : 0, id]
    );
